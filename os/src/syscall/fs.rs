...
use crate::fs::{File, link_file, open_file, OpenFlags, Stat, StatMode, unlink_file};
use crate::mm::{copy_out_data, translated_byte_buffer, translated_str, UserBuffer};
    ...
    if let Some(file) = &inner.fd_table[fd] {
        ...
        match file.inode() {
            Some(inode) => {
                ...
                copy_out_data(token, st as *const u8, &stat);
                0
            },
            None => -1,
        }
...
pub fn sys_linkat(old_name: *const u8, new_name: *const u8) -> isize {
    ...
    if old_name == new_name || new_name.len() > NAME_LENGTH_LIMIT {
        return -1;
    }
    if let Some(old_inode) = open_file(old_name.as_str(), OpenFlags::RDONLY) {
        let inode = old_inode.inode().unwrap();
